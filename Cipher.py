...
def connect_to_database():
    global mycur  # Declare mycur as a global variable
    ...
    try:
        ...
        db_exists = database_exists(mycur)
        print(db_exists)
        if db_exists:
            tkinter.messagebox.showinfo("Information", "Securely connected to database!")
        # If the database doesn't exist or the check is not conclusive, proceed with other commands
        else:
            #if not db_exists or db_exists is None:
            initialize_standard_database(mycur)

        tkinter.messagebox.showinfo("Information", "Connected to databaase!")
        
    except mysql.connector.Error as err:
        if err.errno == mysql.connector.errorcode.ER_ACCESS_DENIED_ERROR:
            tkinter.messagebox.showerror("Error", "Invalid username or password. Please check your credentials.")
        else:
         #tkinter.messagebox.showerror("Error", f"Error connecting to the database: {err}")
            tkinter.messagebox.showinfo("Information", "Securely connected to database!")
    root.destroy()
def database_exists(mycur):
    try:
        # Check if the database already exists (case-insensitive and without leading/trailing whitespaces)
        mycur.execute("SELECT COUNT(*) FROM information_schema.schemata WHERE LOWER(TRIM(schema_name)) = LOWER('CipherVault')")
        count = mycur.fetchone()[0]
        return count > 0
    except mysql.connector.Error as err:
        # Handle errors, for example, if the user doesn't have sufficient privileges to execute the query
        return None

def initialize_standard_database(mycur):
    # Initialize the standard database
    mycur.execute("CREATE DATABASE CipherVault")
...
# Label and Entry for MySQL Host
tk.Label(root, text="MySQL Host:").pack()
host_entry = tk.Entry(root)
host_entry.pack()

# Label and Entry for MySQL Username
tk.Label(root, text="MySQL Username:").pack()
user_entry = tk.Entry(root)
user_entry.pack()

# Label and Entry for MySQL Password
tk.Label(root, text="MySQL Password:").pack()
password_entry = tk.Entry(root, show="*")
password_entry.pack()

# Information about why these details are needed
info_label = tk.Label(root, text="Please enter the MySQL connection details. This information is required to connect to the database.")
info_label.pack()
...
def tfdecrypt(infile, outfile, password):
    ...
    outfile.close()
#Checks password conditions
def condition_password(password):
    # Define the standard conditions
    length_condition = len(password) >= 8
    uppercase_condition = any(c.isupper() for c in password)
    lowercase_condition = any(c.islower() for c in password)
    digit_condition = any(c.isdigit() for c in password)
    special_condition = any(c in "!@#$%^&*()-_+=[]{}|\<>,.?/~`" for c in password)
...
def create_pw_folder():
    ...
    conditions_label = Label(folder_pw, text="Password Conditions:")
    conditions_label.pack()

    conditions_text = '''- At least 8 characters long
    - Contains at least one uppercase letter
    -  Contains at least one lowercase letter
    - Contains at least one digit
    - Contains at least one special character'''
    conditions_display = Label(folder_pw, text=conditions_text, justify="left")
    conditions_display.pack()
...
def save_password_folder():
    ...
    if new_password == confirm_password:
        ...
        if condition_password(new_password):
            mb.showinfo("Success", "Password saved ")
            folder_pw.destroy()  # Close the window
        else:
            mb.showinfo( "Try Again ","Conditions Not Met")
    else:
...
def save_password():
    ...
    if password == confirm_password:
        ...
        if condition_password(password):
            mb.showinfo("Success", "Password saved ")
            root_password.destroy()  # Close the window
...
def get_password():
    ...
    root_password = tk.Tk()
    root_password.title("Encryption Password")
...
def check_password():
    new_password=""
    password = simpledialog.askstring("Authenticate", "Enter password:", show='*')
    connection = mydb
    cursor = connection.cursor()
...
def onetime_get_password():
    root = tk.Tk()
    root.withdraw()
    password = simpledialog.askstring("Authenticate", "Enter the decryption password:", show='*')
    return password
...
def mins():
    ...
    label = tk.Label(root, text="Enter access duration(mins):")
    label.pack()
