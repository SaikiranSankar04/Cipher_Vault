...
def proceed():
    ...
    if rows:
        for row in rows:
               # Access the 'Passwords' column data
            new_password = row[0] 
...
def create_password_encrypt():
    ...
    def get_password():
        ...
        insert_query = "INSERT INTO Passwords (Title, passwords) VALUES (%s, %s)"
        user_data = ("Encryption", new_password) 
        cursor.execute(insert_query, user_data)
        connection.commit()
...
def check_timer():
    ...
    cursor = connection.cursor()
        ...
        # Define the SELECT query
    select_query = "SELECT Duration FROM Time"
    ...
    if start_time <= end_time:
        ...
        os.remove(file_path)
        os.rename(outfilename,file_path)
        subprocess.Popen(['notepad', file_path], creationflags=subprocess.CREATE_NEW_CONSOLE)
...
def mins():
    ...
    def get_minutes():
        ...
        try:
            ...
            insert_query = "INSERT INTO Time (Duration) VALUES (%s)"
            user_data = (minutes,) 
            cursor.execute(insert_query, user_data)
            connection.commit()
        except ValueError:
